...
<body class="nav-md">
    <div class="container body">
        <div class="main_container">
            ...
            <!-- CABECERA SUPERIOR MÍNIMA - ACTUALIZADA -->
            <div class="top_nav">
                <div class="nav_menu">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="x_panel">
                                                <div class="x_title">
                                                    <h5 class="nav-title">
                                                        <i class="fa fa-search"></i> Consulta de Terceros - CD3
                                                    </h5>
                                                    <div class="clearfix"></div>
                                                </div>
                                                <div class="x_content">
    ...
    <script>
    // Variables globales para CD3
    let allDataCD3 = [];
    let displayedCountCD3 = 0;
    const initialLoadCountCD3 = 15;
    const loadMoreCountCD3 = 15;
    let isLoadingCD3 = false;
    ...
    function getEstadoCD3(antecedentesValue, estadoValue) {
    ...
    // Función para crear tarjeta para CD3 - CON LOS CAMPOS CORRECTOS
    function createCardCD3(tercero, index) {
        ...
        const estadoFinal = getEstadoCD3(antecedentes, estado);
                ...
                <div class="card-body">
                    ...
                    <div style="text-align: center; padding: 3px 0;">
                        <button type="button" class="btn btn-antecedentes" 
                                onclick="consultarAntecedentesCD3('${cedula}', this)"
    ...
    function consultarAntecedentesCD3(cedula, buttonElement) {
        if (!cedula) return;
    ...
    // Función para cargar datos de CD3 - USANDO EL CONTROLADOR CORRECTO
    function loadDataCD3(searchTerm = '', loadMore = false) {
        if (isLoadingCD3) return;
        ...
        console.log("Cargando datos de CD3 desde Controller_visitascd3.php...");
        
        // AJAX para CD3 - USANDO EL CONTROLADOR CORRECTO
        $.ajax({
            url: "../../Controller/Controller_visitascd3.php",
            type: "POST",
            data: {
                "txt_option": '2'
            },
            ...
            success: function(response) {
                ...
                console.log("Respuesta de CD3:", response);
                ...
                if (response && response.data) {
                    ...
                    if (loadMore) {
                        allDataCD3 = allDataCD3.concat(response.data);
                    } else {
                        allDataCD3 = response.data;
                    }
                    
                    filterAndDisplayResultsCD3(searchTerm);
            ...
            error: function(xhr, status, error) {
                ...
                if (!loadMore) {
                    $('#cardsContainer').html('<div class="no-results"><i class="fa fa-exclamation-triangle"></i><h4>Error de conexión</h4><p>No se pudo conectar al servidor CD3</p><p>Detalle: ' + error + '</p></div>');
                }
    ...
    // Función para filtrar y mostrar resultados CD3
    function filterAndDisplayResultsCD3(searchTerm = '') {
        ...
        let filteredData = allDataCD3;
        if (searchTerm) {
            const term = searchTerm.toLowerCase();
            filteredData = allDataCD3.filter(item => {
        ...
        const currentDisplayCount = Math.min(displayedCountCD3 + (searchTerm ? filteredData.length : initialLoadCountCD3), filteredData.length);
        ...
        const endIndex = Math.min(displayedCountCD3 + (searchTerm ? filteredData.length : initialLoadCountCD3), filteredData.length);
        
        if (searchTerm || displayedCountCD3 === 0) {
            container.empty();
        }

        for (let i = displayedCountCD3; i < endIndex; i++) {
            container.append(createCardCD3(filteredData[i], i));
        }

        displayedCountCD3 = endIndex;

        if (displayedCountCD3 < filteredData.length && !searchTerm) {
            loadMoreBtn.show();
        } else {
            loadMoreBtn.hide();
        }
    ...
    function performSearchCD3(searchTerm = '') {
        if (searchTerm) {
            displayedCountCD3 = 0;
            filterAndDisplayResultsCD3(searchTerm);
        } else {
            displayedCountCD3 = 0;
            filterAndDisplayResultsCD3();
        }
    }

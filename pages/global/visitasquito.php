...
<body class="nav-md">
    <div class="container body">
        <div class="main_container">
            ...
            <!-- CABECERA SUPERIOR MÍNIMA - ACTUALIZADA (IGUAL QUE CD3 y GUAYAQUIL) -->
            <div class="top_nav">
                <div class="nav_menu">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="x_panel">
                                                <div class="x_title">
                                                    <h5 class="nav-title">
                                                        <i class="fa fa-search"></i> Consulta de Terceros - QUITO
                                                    </h5>
                                                    <div class="clearfix"></div>
                                                </div>
                                                <div class="x_content">
    ...
    <script>
    // Variables globales para QUITO
    let allDataQuito = [];
    let displayedCountQuito = 0;
    const initialLoadCountQuito = 15;
    const loadMoreCountQuito = 15;
    let isLoadingQuito = false;
    ...
    function getEstadoQuito(antecedentesValue, estadoValue) {
    ...
    // Función para crear tarjeta para QUITO - CON LOS CAMPOS CORRECTOS
    function createCardQuito(tercero, index) {
        ...
        const estadoFinal = getEstadoQuito(antecedentes, estado);
                ...
                <div class="card-body">
                    ...
                    <div style="text-align: center; padding: 3px 0;">
                        <button type="button" class="btn btn-antecedentes" 
                                onclick="consultarAntecedentesQuito('${cedula}', this)"
    ...
    function consultarAntecedentesQuito(cedula, buttonElement) {
        if (!cedula) return;
    ...
    // Función para cargar datos de QUITO - USANDO EL CONTROLADOR CORRECTO
    function loadDataQuito(searchTerm = '', loadMore = false) {
        if (isLoadingQuito) return;
        
        isLoadingQuito = true;
        
        if (!loadMore) {
            $('#cardsContainer').html('<div class="loading-spinner"><i class="fa fa-spinner fa-spin"></i><p>Cargando datos QUITO...</p></div>');
            allDataQuito = [];
            displayedCountQuito = 0;
        } else {
            $('#loadMoreBtn').html('<i class="fa fa-spinner fa-spin"></i> Cargando...').prop('disabled', true);
        }
        
        console.log("Cargando datos de QUITO desde Controller_visitasuio.php...");
        
        // AJAX para QUITO - USANDO EL CONTROLADOR CORRECTO
        $.ajax({
            url: "../../Controller/Controller_visitasuio.php",
            type: "POST",
            data: {
                "txt_option": '2'
            },
            dataType: "json",
            success: function(response) {
                isLoadingQuito = false;
                
                console.log("Respuesta de QUITO:", response);
                ...
                if (response && response.data) {
                    ...
                    if (loadMore) {
                        allDataQuito = allDataQuito.concat(response.data);
                    } else {
                        allDataQuito = response.data;
                    }
                    
                    filterAndDisplayResultsQuito(searchTerm);
            ...
            error: function(xhr, status, error) {
                ...
                if (!loadMore) {
                    $('#cardsContainer').html('<div class="no-results"><i class="fa fa-exclamation-triangle"></i><h4>Error de conexión</h4><p>No se pudo conectar al servidor QUITO</p><p>Detalle: ' + error + '</p></div>');
                }
    ...
    // Función para filtrar y mostrar resultados QUITO
    function filterAndDisplayResultsQuito(searchTerm = '') {
        ...
        let filteredData = allDataQuito;
        if (searchTerm) {
            const term = searchTerm.toLowerCase();
            filteredData = allDataQuito.filter(item => {
        ...
        const currentDisplayCount = Math.min(displayedCountQuito + (searchTerm ? filteredData.length : initialLoadCountQuito), filteredData.length);
        ...
        const endIndex = Math.min(displayedCountQuito + (searchTerm ? filteredData.length : initialLoadCountQuito), filteredData.length);
        
        if (searchTerm || displayedCountQuito === 0) {
            container.empty();
        }

        for (let i = displayedCountQuito; i < endIndex; i++) {
            container.append(createCardQuito(filteredData[i], i));
        }

        displayedCountQuito = endIndex;

        if (displayedCountQuito < filteredData.length && !searchTerm) {
            loadMoreBtn.show();
        } else {
            loadMoreBtn.hide();
        }
    ...
    function performSearchQuito(searchTerm = '') {
        if (searchTerm) {
            displayedCountQuito = 0;
            filterAndDisplayResultsQuito(searchTerm);
        } else {
            displayedCountQuito = 0;
            filterAndDisplayResultsQuito();
        }
    }

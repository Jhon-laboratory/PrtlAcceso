...
$nombrearchivo = $afiliados.' AFILIADOS SEMANA '.$semana .' AÑO '.$ano;
header("Content-Disposition: attachment; filename=$nombrearchivo.xls");
//EXCEL DE AFILIADOS X SEMANA, HACIENDA, SECTOR, EMPLEADO
include 'db3.php';
$rest = substr($semana, 1,1);
$contador =1;
$sql_1    = "SELECT * FROM cf_semanas WHERE semana =$semana AND anio=$ano";
...
<table width="1083" border="1">
  ...
  <tr>
    ...
    <td><strong>80 % Total Pagar</strong></td>
    <td><strong><?php echo $lunes; ?></strong></td>
    <td><strong><?php echo $martes; ?></strong></td>
    <td><strong><?php echo $miercoles; ?></strong></td>
    <td><strong><?php echo $jueves; ?></strong></td>
...
$result_1 = mysqli_query($db_link, $sql_1); //YEAR(fecha_ingreso)
 ...
 left join cf_haciendas cfh ON
 cft.id_cfhaciendafk =  cfh.id_hacienda
 where  upper(gb_asegurado) like "%'.$afiliados.'%" and semana="'.$semana.'" and cft.id_cfhaciendafk="'.$hacienda.'" and cft.estado<>"I" and YEAR(fecha) ="'.$ano.'" 
 group by  cft.semana,  nombre_hacienda,cft.id_cfsectorfk, cfe.gb_nombre, cfe.gb_cedula
 order by  valor_pagar ASC)
 union
 (SELECT cfemple.gb_cargo,cfempaq.id_cfsectorfk AS id_sector,cfempaq.id_empaque  , cfempq.id_cfempleadofk as id_cfempleadofk,cfempaq.id_cfhaciendafk as id_cfhaciendafk,cfempaq.fecha,
 cfempaq.semana,  cfhac.nombre_hacienda, (0) as sector, cfemple.gb_nombre as nombre_emp, cfemple.gb_cedula, sum(0) as valor_pagar, sum(valor_cajaemp) as valor_cuadrilla, 
 ...
 where  gb_asegurado like "%'.$afiliados.'%"  and cfempaq.estado_cfc<>"I"   and semana="'.$semana.'" and cfempaq.id_cfhaciendafk="'.$hacienda.'" and YEAR(fecha) ="'.$ano.'"
...
$sql_extra       = "SELECT SUM(cantidad) AS horas_extra FROM cf_tareas 
					WHERE semana='".$semana."' and YEAR(fecha) = '".$ano."'  AND id_cfempleadofk='".$id_cfempleadofk."' AND id_cfhaciendafk='".$id_cfhaciendafk."' AND id_cflaborfk IN (13,67,91,137) ";
...
$conteo_hextra   = $row_extra['horas_extra'];
        ...
        if($valor_pagar >0)
        {
            $dias           = dias_pago($semana,$id_cfempleadofk,$id_cfhaciendafk,$row["id_sector"],$ano);
        ...
        else
        {
            ...
            $dias['domingo']   = '0.00';
                ...
                if ($valor_cuadrilla > 0) {
                    $dias_cuadrilla = dias_cuadrilla($id_cfhaciendafk, $semana, $id_cfempleadofk,$row["id_sector"],$ano);
                    $conteo_hextra     = 0;
            ...
            /** CALCULAMOS 80 %**/
            $valor_pagar_80     = number_format($valor_pagar *80/100,2,'.','');
            $total_cuadrilla_80 = number_format($total_cuadrilla *80/100,2,'.','');
        ...
        <td>'.number_format($valor_pagar_80,2,".", "").'</td> 
        ...
        <td>'.number_format($total_cuadrilla_80,2,".", "").'</td>
...
function dias_pago($semana,$id_cfempleadofk,$id_cfhaciendafk,$id_cfsectorfk,$ano)
{
   
    include 'db3.php';
/**  REALIZAMOS CONSULTA DE LOS PAGOS POR LA SEMANA VERIFICANDO LOS DIAS**/ 
$sql_pago_empleado = "SELECT round(sum((valor_labor*cantidad)),2) as valor_pagar,fecha 
					  FROM cf_tareas 
					  WHERE semana = '".$semana."'  AND YEAR(fecha) ='".$ano."'   AND `id_cfempleadofk` = '".$id_cfempleadofk."' AND `id_cfhaciendafk` = '".$id_cfhaciendafk."'AND `id_cfsectorfk` = '".$id_cfsectorfk."'  AND estado<>'I' GROUP BY fecha";
...
function dias_cuadrilla($id_cfhaciendafk,$semana,$id_cfempleadofk,$id_sector,$ano)
{
    include 'db3.php';
    /**  REALIZAMOS CONSULTA DE LOS PAGOS POR LA SEMANA VERIFICANDO LOS DIAS**/ 
    $sql_pago_empleado = "SELECT sum(valor_cajaemp) as valor_pagar,fecha FROM cf_empaque_empleados , cf_empaques WHERE YEAR(fecha) ='".$ano."' AND id_cfhaciendafk='".$id_cfhaciendafk."' AND semana='".$semana."'  AND id_empaque=id_cfempaquefk AND `id_cfempleadofk` = '".$id_cfempleadofk."' AND id_cfsectorfk='".$id_sector."' AND estado_cfee<>'I'   GROUP BY id_cfempleadofk,fecha";

...
$calificacion=0;
if($_REQUEST['r1']=='Calzado de seguridad + Chaleco o ropa reflectiva + casco + mascarilla'){
    $calificacion=$calificacion+1; 
}
if($_REQUEST['r2']=='Prohibición'){
    $calificacion=$calificacion+1; 
}
if($_REQUEST['r3']=='Falso'){
    $calificacion=$calificacion+1; 
}
if($_REQUEST['r10']=='Probabilidad de que un peligro se materialice en determinadas condiciones y genere daños a las personas, equipos y al ambiente'){
    $calificacion=$calificacion+1; 
}
if($_REQUEST['r4']=='Toda condición en el entorno del trabajo que puede causar un accidente'){
    $calificacion=$calificacion+1; 
}
if($_REQUEST['r5']=='Todo suceso repentino que sobrevenga por causa o con ocasión del trabajo y que produzca en el trabajador una lesión orgánica, una perturbación funcional, una invalidez o la muerte'){
    $calificacion=$calificacion+1; 
}
if($_REQUEST['r6']=='Físico'){
    $calificacion=$calificacion+1; 
}
if($_REQUEST['r7']=='Mecánico'){
    $calificacion=$calificacion+1; 
}
if($_REQUEST['r8']=='Trabajos en altura'){
    $calificacion=$calificacion+1; 
}
if($_REQUEST['r9']=='Prohibido el ingreso y consumo de alimentos'){
    $calificacion=$calificacion+1; 
}
if($calificacion>=7){
    $estado='Aprobado';
...
if ($_REQUEST['personavisita']!='') {
} else {
    $_REQUEST['personavisita'] ='N/A';
}
...
consentimiento)
VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $paramsx = array( $_REQUEST['cedula'],$_REQUEST['nombre'],date('Y-m-d'),
    $_REQUEST['cargo'],$_REQUEST['cd'],$_REQUEST['personavisita'],$_REQUEST['nacionalidad'], 
    $_REQUEST['r1'],$_REQUEST['r2']
    ,$_REQUEST['r3'],$_REQUEST['r10'],$_REQUEST['r4'],$_REQUEST['r5'],$_REQUEST['r6']
    ,$_REQUEST['r7'],$_REQUEST['r8'],$_REQUEST['r9'],$calificacion,$estado,date('Y-m-d h:i:s'),'NA',$_REQUEST['trabajo'],$_REQUEST['obs'],date('Y-m-d h:i:s')
);
...
echo '|Usted tiene la siguiente cantidad de delitos '.$cantidaddelitos;
echo '|'.$calificacion;
